...
function listClientes(){
  ...
  for (let j = 0; j < icontadorcliente; j++) {
    console.log("Código do cliente:", j , "Nome do cliente:", nomes[j] , "CPF:" ,cpfs[j] ,"STATUS: ", SitCliente[j] ? "Esta com o livro" : "Nao esta com o livro")
    }
    setTimeout(() => {
        menu()
      }, 1000)
...
function listLivro(){
    ...
    for(let j = 0; j <icontadorlivro; j++){
        console.log('Código do livro: ',j, 'Nome do livro: ',livros[j], 'STATUS:', SitLivro[j] ? 'Emprestado' : 'Disponivel' )
    }
    setTimeout(() => {
        menu()
      }, 1000)
...
function emprestLivro() {
  let numeroDoLivro = leitor.question('Digite o código do livro que quer pegar emprestado: ');
  let numeroDoCliente = leitor.question('\nDigite o código do cliente: ');

  if (numeroDoLivro < 0 || numeroDoLivro >= icontadorlivro || numeroDoCliente < 0 || numeroDoCliente >= icontadorcliente) {
